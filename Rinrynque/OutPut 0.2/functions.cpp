...
void outPut::display(Matrix& array)
{
...
// OpenGL rendering goes here...
    ...
    for(unsigned int x = 0; x < array.GetX(); x++)
    {
        for(unsigned int y = 0; y < array.GetY(); y++)
        {
            ...
            {
               if(array.GetValue(x,y,i,3))
            glVertex2d(x*pixelSize+pixelSize/2.0+array.GetValue(x,y,i,0)*pixelSize/3.0, y*pixelSize+pixelSize/2.0+array.GetValue(x,y,i,1)*pixelSize/3.0);
