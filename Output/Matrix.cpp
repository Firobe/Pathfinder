...
int Matrix::GetValue(int x, int y, int d, int i)
{
...
if(data)
{
    ...
    for(unsigned int j=0 ; j < height ; j++)
    {
        ...
        {
            pixel=0;
            for(unsigned int i=0; i<3; i++) //Pour chaque octet de chaque pixel recommencer (3 fois car 24 bits)
            {
                data>>carac;
                pixel+=carac;
            }
            pixel=pixel/3;
            _array[i][j][0][0]=pixel;
        }
