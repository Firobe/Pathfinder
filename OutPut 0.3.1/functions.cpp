...
outPut::outPut(const Matrix array)
{
    _data = array;
}
...
// OpenGL rendering goes here...
        ...
        glBegin(GL_POINTS);
        for(unsigned int x = 0; x < (unsigned)_data.GetX(); x++)
        {
            for(unsigned int y = 0; y < (unsigned)_data.GetY(); y++)
            {
                for(unsigned int i = 1; i < 9; i++)
                {
                    if(_data.GetValue(x,y,i))
...
void outPut::genList()
{
    ...
    for(unsigned int x = 0; x < (unsigned)_data.GetX(); x++)
    {
        for(int y = 0; y < _data.GetY(); y++)
        {
            ...
            fprintf(out, "%d|", _data.GetValue(x,y,0));
        }
        fprintf(out, "\n");
    }
    glEnd();
